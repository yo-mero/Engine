...
// Método para inicializar una textura a partir de un archivo de imagen
HRESULT Texture::init(Device device, std::string TextureName, ExtensionType extensionType)
{
	...
	HRESULT hr = S_OK;
		...
		{
			// Caso para cargar una textura DDS
		case DDS:
			...
			break;
			// Caso para cargar una textura PNG
		case PNG:
...
// Método para inicializar una textura vacía con dimensiones específicas
	void Texture::init(Device device, unsigned int width, unsigned int height, DXGI_FORMAT Format, unsigned int Bindflags)
{
	...
	// Método para renderizar la textura
	...
	// Método para destruir la textura y liberar recursos
